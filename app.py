...
def compress_pdf(input_path, output_path):
    """
    Compress PDF - Compatible with older PyMuPDF versions
    Uses page-by-page rebuild method to save RAM on 512MB server
    """
    ...
    try:
        ...
        for page_num in range(total_pages):
            # Force GC setiap halaman untuk hemat RAM
            gc.collect()
            
            page = src_doc[page_num]
            
            # Create new page dengan ukuran sama
            new_page = dst_doc.new_page(width=page.rect.width, height=page.rect.height)
            
            # Copy content dari source page
            new_page.show_pdf_page(new_page.rect, src_doc, page_num)
            
            logger.debug(f"Processed page {page_num + 1}/{total_pages}")
        
        # Close source untuk free RAM
        src_doc.close()
        src_doc = None
        gc.collect()
        
        # Save dengan kompresi maksimal
        # FIX: linear=False untuk avoid "Linearisation is no longer supported"
        try:
            dst_doc.save(
                output_path,
                ...
                linear=False    # Disable linearisation
            )
        except TypeError:
        ...
        logger.info(f"Compression complete: {original_size} -> {compressed_size} bytes ({reduction:.1f}% reduction)")
        ...
        if dst_doc:
            try:
                ...
                pass
        # Jika gagal, copy file asli sebagai fallback
        try:
            shutil.copy2(input_path, output_path)
            logger.warning("Compression failed, copied original file")
        except:
            raise e
    finally:
        gc.collect()
